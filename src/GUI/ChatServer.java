    ...
    private class Handler extends Thread {
        ...
        private void kickUser(String username) {
            ...
            if (toKick != null) {
                ...
                out.println("[SERVER]> User '" + username + "' not found or is the root user.");
            }
        }
        ...
        private void banUser(String username) {
            ...
            if (userHandler != null && !isRoot(userHandler.SessionID, userHandler.username)) {
                String ipToBan = getUserIPByUsername(username);

                if (!bannedIPs.contains(ipToBan)) {
                    bannedIPs.add(ipToBan);
                    Handlers.stream()
                            .filter(handler -> getUserIPByUsername(username).equalsIgnoreCase(username))
                            .forEach(Handler::closeConnection);
                    out.println("[SERVER]> IP '" + ipToBan + "' associated with username '" + username + "' has been banned.");
                } else {
