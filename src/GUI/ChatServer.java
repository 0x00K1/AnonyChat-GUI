...
public class ChatServer extends ChatServerGUI {
	...
	public ChatServer(String serverIP, int serverPort, String serverName, int serverCapacity, boolean isPrivate, String serverPass, String userRoot) {
    ...
    private class Handler extends Thread {
        ...
        private boolean authenticator(Socket socket, BufferedReader in, PrintWriter out) throws IOException {
        	...
        	try {
        		if (isPrivate) {
        			...
        			String serverPassword = serverCredentials.get("passwordHash");
	        		...
	        		if (serverPassword == null || serverPassword.trim().isEmpty()) {
	        			...
	        			if (clientServerPassword == null || !SecurityUtils.validatePassword(clientServerPassword, serverSalt, serverPassword)) {
        		...
        		if (GETusername == null || GETusername.trim().isEmpty()) {
        		    ...
        		    return false;
        		} else if (isUserOnline(GETusername, serverIP, serverPort, userRoot)) {
				...
				if (Root.equalsIgnoreCase(GETusername)) {
                    ...
                    String Rpassword = credentials.get("passwordHash");
                    String storedSalt = credentials.get("salt");
        			if (clientRootPassword == null || !SecurityUtils.validatePassword(clientRootPassword, storedSalt,  Rpassword)) {
        ...
        private void serverInfo() {
            String info = String.format(
                "[SERVER]> Server Info: IP: %s - Port: %d - Private: %s - SSL Enabled: %s\n",
                serverIP,
                serverPort,
                isPrivate ? "Yes" : "No",
                isSSLEnabled() ? "Yes" : "No"
        ...
        private void togglePrivacy(String privacy) {
            if ("private".equals(privacy) && !isPrivate) {
                isPrivate = true;
                ...
                String hashedPassword = SecurityUtils.hashPassword(newServerPass, salt);
                serverPass = hashedPassword;
                updateServerPassword(serverIP, serverPort, userRoot, serverPass, salt, isPrivate);
                out.println("[SERVER]> Server set to private.");
            } else if ("public".equals(privacy) && isPrivate) {
                isPrivate = false;
                // Clear password and salt when setting to public
                serverPass = null;
                updateServerPassword(serverIP, serverPort, userRoot, serverPass, null, isPrivate);
                out.println("[SERVER]> Server set to public.");
            } else if (privacy.equals("reset") && isPrivate) {
                // Reset the password for a private server
                String newServerPass = passwordInput("Enter new server password:");
                if (newServerPass.length() < 4 || newServerPass.length() > 99) {
                    out.println("[SERVER]> Server Password must be between 4 and 99 characters.");
                    return;
                }
                ...
                serverPass = hashedPassword;
                updateServerPassword(serverIP, serverPort, userRoot, serverPass, salt, isPrivate);
                out.println("[SERVER]> Server password reset.");
            } else if (isPrivate && "private".equals(privacy)) {
                out.println("[SERVER]> Server is already set to private.");
            } else if (!isPrivate && "public".equals(privacy)) {
                out.println("[SERVER]> Server is already set to public.");
            } else {
                out.println("[SERVER]> Invalid privacy setting. Use 'private', 'public', or 'reset'.");
            }
        }
